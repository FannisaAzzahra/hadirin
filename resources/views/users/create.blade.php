        ...
        <div class="col-lg-8">
            <div class="hadirin-app-card">
                <div class="hadirin-app-header">
                    <h5 class="hadirin-header-title">Tambah User Baru</h5>
                </div>
                <div class="hadirin-app-body">
                    <div class="hadirin-section-header">
                        <i class="fas fa-user-plus hadirin-section-icon"></i>
                        <h6 class="hadirin-section-title">Informasi Akun Baru</h6>
                    </div>
                    <form action="{{ route('users.store') }}" method="POST">
                        @csrf
                        <div class="form-row mb-3">
                            <label for="name" class="hadirin-label">Nama <span class="required-star">*</span></label>
                            <input type="text" class="hadirin-input @error('name') is-invalid @enderror" id="name" name="name" value="{{ old('name') }}" placeholder="Masukkan nama" required>
                            @error('name')
                        ...
                        <div class="form-row mb-3">
                            ...
                            <input type="email" class="hadirin-input @error('email') is-invalid @enderror" id="email" name="email" value="{{ old('email') }}" placeholder="user@example.com" required>
                            @error('email')
                                <div class="invalid-feedback">⚠️ {{ $message }}</div>
                            @enderror
                        </div>
                        <div class="form-grid password-fields">
                            <div class="form-row mb-3">
                                <label for="password" class="hadirin-label">Password <span class="required-star">*</span></label>
                                <input type="password" class="hadirin-input @error('password') is-invalid @enderror" id="password" name="password" placeholder="Minimal 8 karakter" required>
                                @error('password')
                                    <div class="invalid-feedback">⚠️ {{ $message }}</div>
                                @enderror
                            </div>
                            <div class="form-row mb-3">
                                <label for="password_confirmation" class="hadirin-label">Konfirmasi Password <span class="required-star">*</span></label>
                                <input type="password" class="hadirin-input" id="password_confirmation" name="password_confirmation" placeholder="Konfirmasi password baru" required>
                            </div>
                        </div>

                        <div class="action-buttons">
                            <a href="{{ route('users.index') }}" class="hadirin-btn-secondary">Batal</a>
                            <button type="submit" class="hadirin-btn-primary">Simpan</button>
                        </div>
                    </form>
                </div>
            </div>
